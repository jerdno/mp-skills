...
- **[implement-with-demonstrate](./implement-with-demonstrate/SKILL.md)** — Implement a spec or ticket with `/implement`, then QA it like a human tester with `/demonstrate`, at most twice, with one fix round in between. The PR ends up carrying the verdict. Needs `implement` and `demonstrate` installed. User-invoked.
