...
1. **The project's `docs/agents/demonstrate.md`** — the manual-testing record a previous invocation wrote, and anything it points to.
...
Every answer found below rung 1 gets **recorded** in the project's `docs/agents/demonstrate.md` (create it if missing), so the next invocation self-serves at rung 1. A skill-owned file, deliberately not a CLAUDE.md section: this knowledge is needed only when demonstrating, so it must not ride in every conversation's context. Record pointers, never secret values — the file is committed: `credentials in .env.local under TEST_USER_*` belongs there; the password itself never does. Shape it like:
...
Completion criterion: start, exercise, auth, and wiring answered for every surface the intent touches, and every newly-learned answer persisted to `docs/agents/demonstrate.md`.
...
- A blocker rooted in a stale `docs/agents/demonstrate.md` record heals the record before re-dispatch.
...
- When a finding reports that the recorded environment knowledge failed — the documented auth method no longer works, the entry point moved — get the correction from the user and update `docs/agents/demonstrate.md` before finishing, so the record heals instead of rotting.
