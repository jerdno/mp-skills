...
- **[api-contract-change](./api-contract-change/SKILL.md)** — Walk through a contract-first API change (OpenAPI or GraphQL) across separate schemas and consumer repos.
...
- **[implement](./implement/SKILL.md)** — Build the work described by a spec or set of tickets in its own git worktree, driving `/tdd` at pre-agreed seams, closing out with a bounded `/code-review` fix loop, then opening the pull request, or stopping short of it with `--no-pr`.
