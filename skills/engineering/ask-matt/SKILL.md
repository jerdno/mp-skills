...
3. **Branch — is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally — one red-green slice at a time — commits the green build, then closes out with a bounded fix loop: up to three rounds of **`/code-review`**, a two-axis review (Standards + Spec) of the run's diff, fixing hard findings (and the judgement calls it deems relevant) and committing after each round. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
