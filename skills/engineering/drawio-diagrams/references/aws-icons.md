...
> Catalogue adapted and translated from the [softaworks/agent-toolkit](https://github.com/softaworks/agent-toolkit) `draw-io` skill (MIT).
...
- Avoid bare abbreviations (ECS → Amazon ECS)
- Use official service names

## 2. Icon styles

### 2.1. Resource icons (the default form)
...
- Use `mxgraph.aws4.*` (aws3 is the old set — avoid it)
- Service names are given in snake_case
...
| Machine Learning (generic) | mxgraph.aws4.machine_learning |
...
Search this catalogue without loading it whole: `python3 scripts/find-aws-icon.py <service>` ([find-aws-icon.py](../scripts/find-aws-icon.py)).
