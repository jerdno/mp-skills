...
`SKILL.md`'s rule governs (pick once, apply consistently, ask the user once per project). When the user has no existing palette, the canonical **C4-PlantUML palette** is a fine default — the four–five categories it distinguishes (person, in-scope system, external system, container, store) are exactly the ones a C4 diagram needs.
