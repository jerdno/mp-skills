...
- **[api-contract-change](./skills/engineering/api-contract-change/SKILL.md)** — Walk through a contract-first API change (OpenAPI or GraphQL) across separate schemas and consumer repos: bump the contract, open the PR to trigger artifact generation, wait for SNAPSHOT, then update each consumer — the service for REST; server resolvers and client operations for GraphQL.
- **[drawio-diagrams](./skills/engineering/drawio-diagrams/SKILL.md)** — Lay out publication-ready draw.io diagrams on the first render: edge routing that keeps labels readable, multiple views as tabs in one file, a bundled geometric overlap checker for static exports, C4 stencil templates when the view is a C4 one, and official AWS icon lookup for architecture views.
...
- **[implement](./skills/engineering/implement/SKILL.md)** — Build the work described by a spec or set of tickets in its own git worktree, driving `/tdd` at pre-agreed seams, closing out with a bounded `/code-review` fix loop, then opening the pull request, or stopping short of it with `--no-pr`.
