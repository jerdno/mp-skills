...
The skill itself is [harness](https://www.aihero.dev/ai-coding-dictionary/harness)-agnostic — each axis runs on whatever sub-agent mechanism the harness provides. Under Claude Code specifically, a companion reference routes both axes through the OpenAI Codex plugin's `codex:codex-rescue` agent whenever that plugin is installed, falling back to built-in general-purpose sub-agents when the plugin is missing, isn't set up, or a run fails. Codex is the default engine there, not a hard requirement.
